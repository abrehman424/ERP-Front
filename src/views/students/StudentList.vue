<template>
  <div class="relative w-full min-h-screen pl-0 pr-0 sm:pl-0 sm:pr-0 lg:pl-1 lg:pr-0 py-2 sm:py-3 lg:py-4">
    ...
    <div class="relative z-10 w-full mb-6 flex justify-end">
      <div class="flex bg-white rounded-full shadow-lg p-1 border border-gray-200">
        <template v-for="(tab, idx) in viewTabs" :key="tab.key">
          <button @click="setViewMode(tab.key)" :class="[
            ...
            <span class="relative z-10 flex items-center gap-2">
              <svg v-if="tab.key === 'student'" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              <svg v-else-if="tab.key === 'class'" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5" />
              </svg>
              <svg v-else-if="tab.key === 'family'" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
              ...
              <span v-if="tab.count !== null"
                class="px-2 py-0.5 rounded-full text-xs font-bold bg-white text-purple-700 border border-purple-200 ml-1">
                {{ tab.count }}
              </span>
            </span>
            <div v-if="viewMode === tab.key"
              class="absolute inset-0 bg-gradient-to-r from-purple-400 to-pink-400 opacity-20 animate-pulse rounded-full">
            </div>
          </button>
          <div v-if="idx < viewTabs.length - 1" class="h-6 w-px bg-purple-200 mx-1 self-center"></div>
        </template>
      </div>
    </div>
    ...
    <!-- Top Bar Header -->
    <div
      class="relative z-10 w-full mt-2 mb-4 md:flex gap-2 items-center justify-between bg-white/90 rounded-lg shadow-lg py-2 px-5  transition-all duration-500 border border-gray-200">
      ...
      <h2
        class="text-sm sm:text-base font-medium text-gray-900 tracking-tight flex items-center justify-center md:justify-start gap-2 text-center md:text-left">
    ...
    <!-- Filter Bar in Card -->
    <div
      class="relative z-10 w-full rounded-lg transition-all duration-500 ease-in-out transform hover:shadow-lg bg-white p-4">
      <form class="w-full" @submit.prevent="handleSearch">
          ...
          <div class="flex flex-col w-full sm:w-auto sm:min-w-[160px] sm:max-w-[200px] lg:flex-1">
            ...
            <div class="relative">
              <select v-model="selectedSection"
                class="h-9 border-0 bg-transparent px-0 text-sm font-light text-gray-600 focus:ring-0 focus:outline-none w-full appearance-none pr-8">
        ...
        <div class="flex justify-end mt-3">
          <div class="flex items-center gap-2">
            ...
            <button type="button" @click="handleAddStudent"
    ...
    <div class="relative z-10 w-full flex items-center justify-between mb-6 mt-8">
      ...
      <div class="flex-1 flex justify-center">
        <div class="relative w-full max-w-xs flex items-center">
          ...
          <input v-model="searchQuery" @keyup.enter="handleSearch" type="text" placeholder="QUICK SEARCH"
      ...
      <div class="border border-gray-200 rounded-xl p-8 bg-white/40 shadow-sm">
        <div class="flex flex-col items-center justify-center py-12">
          ...
          <div class="relative mb-6">
            ...
            <div
              class="absolute inset-0 w-16 h-16 border-4 border-transparent border-r-indigo-500 rounded-full animate-ping opacity-20">
            </div>
          </div>
          <div class="text-center">
            <p class="text-gray-600 font-medium text-lg mb-1">{{ filtersLoading ? 'Loading Filters' : 'Loading Students'
              }}
            </p>
    ...
    <div v-else class="relative z-10 w-full">
      <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div v-else class="hidden md:block overflow-x-auto">
          <!-- MOBILE CARD VIEW -->
          <div v-if="students.length && true" class="block md:hidden space-y-3">
            <div v-for="student in students" :key="student.id"
              class="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
              <!-- Header -->
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <img v-if="getProfilePicture(student)" :src="getProfilePicture(student)"
                ...
                <!-- Expand -->
                <button @click="toggleStudentDetails(student.id)" class="p-2 rounded-full hover:bg-gray-100">
                  <svg class="w-4 h-4 transition-transform" :class="{ 'rotate-90': expandedStudents[student.id] }"
              ...
              <div class="mt-3 grid grid-cols-3 gap-2">
                <button @click="handleView(student)" class="btn-mobile-view">View</button>
                <button @click="handleEdit(student)" class="btn-mobile-edit">Edit</button>
                <button @click="handleDelete(student)" class="btn-mobile-delete">Delete</button>
              </div>

              <!-- Expanded Details -->
              <div v-if="expandedStudents[student.id]" class="mt-4 text-xs text-gray-700 space-y-2">
                <div><strong>Section:</strong> {{sections.find(s => s.id === student.section_id)?.name || 'N/A'}}
                </div>
                <div><strong>Email:</strong> {{ student.email || 'N/A' }}</div>
                <div><strong>Phone:</strong> {{ student.contact_info?.phone || 'N/A' }}</div>
                <div><strong>Address:</strong> {{ student.contact_info?.address || 'N/A' }}</div>
              </div>
            </div>
          </div>
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gradient-to-r from-indigo-50 to-purple-50">
              <tr>
                <th v-for="col in tableColumns" :key="col.key" @click="handleSort(col.key)"
                  class="py-2 px-2 font-medium text-gray-600 text-sm tracking-wider cursor-pointer select-none group transition-all duration-150 hover:bg-purple-50 border-b-2 border-purple-400 relative #ffffffspace-nowrap">
                  <span class="flex items-center gap-1">
                    {{ col.label }}
                    <span class="flex flex-col ml-1">
                      <svg v-if="sortBy === col.key && sortDirection === 'asc'"
                        class="w-3 h-3 text-purple-500 transform -rotate-180" fill="none" stroke="currentColor"
                        stroke-width="1.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14m0 0l-5-5m5 5l5-5" />
                      </svg>
                      <svg v-else-if="sortBy === col.key && sortDirection === 'desc'" class="w-3 h-3 text-purple-500"
                        fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14m0 0l-5-5m5 5l5-5" />
                      </svg>
                      <svg v-else class="w-3 h-3 text-gray-500" fill="none" stroke="currentColor" stroke-width="1.5"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14m0 0l-5-5m5 5l5-5" />
                      </svg>
                    </span>
                  ...
                  <span
                    class="absolute left-0 bottom-0 w-full h-0.5 bg-purple-100 opacity-0 group-hover:opacity-100 transition"></span>
                </th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <template v-for="(student, idx) in students" :key="student.id">
                <tr
                  :class="['hover:bg-indigo-50 transition-colors', 'border-b border-purple-200', idx === students.length - 1 ? 'border-b-2 border-purple-400' : '']">
                  <td class="py-2 px-2 text-gray-900 font-normal text-xs #ffffffspace-nowrap">
                    <div class="flex items-center gap-2">
                      <div class="relative w-7 h-7">
                        <img v-if="getProfilePicture(student)" :src="getProfilePicture(student)"
                          :alt="student.first_name" class="w-7 h-7 rounded-full object-cover border border-gray-200"
                      ...
                      <button @click="student && toggleStudentDetails(student.id)"
                        class="p-1 rounded-full hover:bg-white transition-colors duration-200" title="Toggle Details">
                        <svg class="w-4 h-4 text-gray-600 transition-transform duration-200"
                          :class="{ 'rotate-90': expandedStudents[student.id] }" fill="none" stroke="currentColor"
                      ...
                      <div>
                        ...
                        <div v-if="student.email" class="text-xs text-gray-900/50 leading-tight">{{ student.email }}
                        </div>
                      </div>
                    </div>
                  </td>
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="bg-green-50 rounded-lg p-3">
                          <h4 class="font-semibold text-gray-900 mb-2 flex items-center text-sm">
                            <svg class="w-4 h-4 mr-2 text-green-600" fill="none" stroke="currentColor" stroke-width="2"
                              viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round"
                          ...
                          <div class="space-y-3 text-xs">
                            <div class="flex justify-between pb-2 border-b-2 border-purple-200">
                              ...
                              <span class="font-medium">{{sections.find(s => s.id === student.section_id)?.name ||
                                'N/A' }}</span>
                            </div>
                            <div class="flex justify-between pb-2 border-b-2 border-green-200">
                        ...
                        <div class="bg-purple-50 rounded-lg p-3">
                          <h4 class="font-semibold text-gray-900 mb-2 flex items-center text-sm">
                            <svg class="w-4 h-4 mr-2 text-purple-600" fill="none" stroke="currentColor" stroke-width="2"
                              viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round"
                          ...
                          <div class="space-y-3 text-xs">
                            ...
                            <div class="flex justify-between pb-2 border-b-2 border-green-200">
                              ...
                              <span class="font-medium">{{ student.family_info?.guardian_phone ||
                                student.family_info?.father_phone || 'N/A' }}</span>
                            </div>
                            <div class="flex justify-between pb-2 border-b-2 border-purple-200">
          ...
          <div v-if="students.length === 0" class="py-12">
            <div class="flex flex-col items-center justify-center">
              <div class="relative mb-4">
                <div
                  class="w-16 h-16 bg-gradient-to-br from-purple-100 to-indigo-100 rounded-full flex items-center justify-center">
                  <svg class="w-8 h-8 text-purple-300" fill="none" stroke="currentColor" stroke-width="2"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
              ...
              <div class="text-center max-w-xs mx-auto">
                ...
                <div class="text-gray-500 text-base font-light mb-2">We couldn't find any students matching your current
                  filters.</div>
      ...
      <div class="flex flex-row items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-4">
          <label class="text-sm text-gray-600 font-medium flex items-center gap-1">
            Per page:
            <select v-model="pageSize" @change="handleCurrentChange(1)"
          ...
          <label class="text-sm text-gray-600 font-medium flex items-center gap-1 mr-3">
            ...
            <input v-model.number="pageInput" type="number" min="1" :max="totalPages"
              class="w-16 rounded-md border border-purple-200 px-2 py-1 text-xs font-semibold text-purple-700 focus:ring-2 focus:ring-purple-300"
              @keyup.enter="goToPage(pageInput)" />
            <button @click="goToPage(pageInput)"
              class="ml-1 px-2 py-1 rounded-md bg-purple-500 text-white text-xs font-semibold hover:bg-purple-600 transition-all flex items-center gap-1">
      ...
      <div class="flex justify-center">
        <div class="flex items-center gap-2">
          <button
            ...
            :disabled="currentPage === 1" @click="handleCurrentChange(currentPage - 1)" aria-label="Previous">
            <span>&larr;</span>
          </button>
          <span
            class="w-9 h-9 flex items-center justify-center rounded-lg bg-purple-600 text-white text-base font-bold shadow-md">
            {{ currentPage }}
          </span>
          <button
            class="w-9 h-9 flex items-center justify-center rounded-lg bg-purple-50 text-purple-500 text-xl font-bold shadow-sm transition-all duration-200 hover:bg-purple-100 focus:outline-none focus:ring-2 focus:ring-purple-300 disabled:opacity-40 disabled:cursor-not-allowed"
            :disabled="currentPage === totalPages" @click="handleCurrentChange(currentPage + 1)" aria-label="Next">
            <span>&rarr;</span>
          </button>
        </div>
      </div>
    ...
    <div v-if="showDetailsPanel" class="fixed inset-0 z-50 overflow-hidden" @click="closeDetailsPanel">
      <div class="absolute inset-0 bg-black bg-opacity-50 transition-opacity duration-300"></div>
      <div
        class="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl transform transition-transform duration-300 ease-in-out">
        <div class="flex flex-col h-full">
          ...
          <div v-else-if="selectedStudentDetails" class="flex-1 overflow-y-auto p-6 space-y-6">
            ...
            <div class="text-center">
              <div class="relative mx-auto w-24 h-24 mb-4">
                <img v-if="getProfilePicture(selectedStudentDetails)" :src="getProfilePicture(selectedStudentDetails)"
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg">
                <div class="text-2xl font-bold text-green-600">{{selectedStudentDetails.section?.name ||
                  sections.find(s =>
                    s.id === selectedStudentDetails.section_id)?.name || 'N/A' }}</div>
            ...
            <div class="bg-white rounded-lg p-4">
              <h5 class="font-semibold text-gray-900 mb-3 flex items-center">
                <svg class="w-5 h-5 mr-2 text-indigo-600" fill="none" stroke="currentColor" stroke-width="2"
                  viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round"
                    d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                </svg>
              ...
              <div class="space-y-3">
                ...
                <div class="flex justify-between">
                  <span class="text-gray-600">Address:</span>
                  <span class="font-medium text-right">{{ selectedStudentDetails.contact_info?.address || 'N/A'
                    }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-lg p-4">
              ...
              <div class="space-y-3">
                <div class="flex justify-between">
                  <span class="text-gray-600">Section:</span>
                  <span class="font-medium">{{selectedStudentDetails.section?.name || sections.find(s => s.id ===
                    selectedStudentDetails.section_id)?.name || 'N/A' }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-600">Admission Date:</span>
            ...
            <div v-if="selectedStudentDetails.family_info" class="bg-white rounded-lg p-4">
              ...
              <div class="space-y-3">
                ...
                <div class="flex justify-between">
                  <span class="text-gray-600">Guardian Phone:</span>
                  <span class="font-medium">{{ selectedStudentDetails.family_info.guardian_phone ||
                    selectedStudentDetails.family_info.father_phone || 'N/A' }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-gray-600">CNIC:</span>
          ...
          <div class="p-6 bg-white border-t">
            <div class="flex gap-3">
              <button @click="handleEdit(selectedStudentDetails)"
              ...
              <button @click="handleView(selectedStudentDetails)"
